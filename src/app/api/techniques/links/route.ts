import getDb from "@/lib/db";
import { NextRequest } from "next/server";

export async function GET(request: NextRequest) {
  const { searchParams } = new URL(request.url);
  const techniqueId = searchParams.get("techniqueId");
  if (!techniqueId) {
    return Response.json({ error: "techniqueId is required" }, { status: 400 });
  }
  const db = getDb();
  const books = db
    .prepare(
      `SELECT b.* FROM books b
       JOIN technique_books tb ON tb.book_id = b.id
       WHERE tb.technique_id = ?`
    )
    .all(techniqueId);
  const equipment = db
    .prepare(
      `SELECT e.* FROM equipment e
       JOIN technique_equipment te ON te.equipment_id = e.id
       WHERE te.technique_id = ?`
    )
    .all(techniqueId);
  return Response.json({ books, equipment });
}

export async function POST(request: NextRequest) {
  const body = await request.json();
  const { techniqueId, type, targetId } = body;
  if (!techniqueId || !type || !targetId) {
    return Response.json({ error: "techniqueId, type, and targetId are required" }, { status: 400 });
  }
  const db = getDb();
  if (type === "book") {
    db.prepare("INSERT OR IGNORE INTO technique_books (technique_id, book_id) VALUES (?, ?)").run(
      techniqueId,
      targetId
    );
  } else if (type === "equipment") {
    db.prepare(
      "INSERT OR IGNORE INTO technique_equipment (technique_id, equipment_id) VALUES (?, ?)"
    ).run(techniqueId, targetId);
  } else {
    return Response.json({ error: "type must be 'book' or 'equipment'" }, { status: 400 });
  }
  return Response.json({ ok: true }, { status: 201 });
}

export async function DELETE(request: NextRequest) {
  const { searchParams } = new URL(request.url);
  const techniqueId = searchParams.get("techniqueId");
  const type = searchParams.get("type");
  const targetId = searchParams.get("targetId");
  if (!techniqueId || !type || !targetId) {
    return Response.json({ error: "techniqueId, type, and targetId are required" }, { status: 400 });
  }
  const db = getDb();
  if (type === "book") {
    db.prepare("DELETE FROM technique_books WHERE technique_id = ? AND book_id = ?").run(
      techniqueId,
      targetId
    );
  } else if (type === "equipment") {
    db.prepare("DELETE FROM technique_equipment WHERE technique_id = ? AND equipment_id = ?").run(
      techniqueId,
      targetId
    );
  }
  return Response.json({ ok: true });
}
